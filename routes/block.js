...
router.post('/user', adminAuth, async (req, res) => {
  ...
  try {
    //UPDATE
    const udpateUserSql =
      'UPDATE user_tb SET is_delete = CURRENT_TIMESTAMP, block_state = CURRENT_TIMESTAMP WHERE email = $1';
...
router.delete('/user', adminAuth, async (req, res) => {
  ...
  try {
    //UPDATE
    const udpateUserSql =
      'UPDATE user_tb SET is_delete = NULL, block_state = NULL WHERE email = $1';
    await pgPool.query(udpateUserSql, [blockEmail]);
  } catch (err) {
    console.log(err);
